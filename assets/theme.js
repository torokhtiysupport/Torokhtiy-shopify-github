...
window.Meganav = (function() {
  ...
  Meganav.prototype.init = function() {
    ...
    var self=this; //saving scope
    // added code to detect mobile and prevent delay in clicks on mobile
    if(!!('ontouchstart' in window)){//check for touch device
  		//$('myElement').unbind('click mouseenter mouseleave'); //use off if you used on, to unbind usual listeners
  		$openBtn.on('click', $.proxy(self.requestMeganav, self))
    //slide down code
    } else {
    	$openBtn.on('mouseenter', $.proxy(self.requestMeganav, self))
    }
    //$openBtn.on('mouseenter', $.proxy(this.requestMeganav, this))
    //$openBtn.on('mouseenter', $.proxy(this.requestMeganav, this)); // changed 'click' with 'mouseenter' and added this line
    //$openBtn.on('focus', $.proxy(this.requestMeganav, this));//changed click for focus
  };
  ...
  Meganav.prototype.requestMeganav = function(evt) {
    ...
    if ($el.hasClass(this.config.activeClass)) {
      if ($el.is('a')) { //added to control which event opens link
  ...
  Meganav.prototype.bindEvents = function() {
    ...
    this.cache.$page.on('mouseleave.meganav', $.proxy(this.close, this)); //added the line
    //this.cache.$page.on('focus.meganav', $.proxy(this.close, this)); //changed click for focus
...
window.QtySelector = (function() {
  ...
  QtySelector.prototype.updateCartItemCallback = function(cart) {
    ...
    if (cart.item_count === 0) {
      localStorage.setItem('full_cost_for_free', cart.total_price);
      location.reload();
      return;
    }
    ...
    var event = new CustomEvent('full_cost_for_free_change', { 'detail': cart.total_price });
...
theme.stickyHeader = (function() {
  ...
  function stickNav() {
    ...
    cache.$stickyBar.addClass(config.navClass);
     cache.$stickyBar.addClass("shadow-1"); //added shadow
  ...
  function unstickNav() {
    ...
    cache.$stickyBar.removeClass('shadow-1') //removing shadow
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _updateAddToCartBtn: function(evt) {
      ...
      if (variant) {
        ...
        let soldOutDiv = jQuery('#sold-out'); //mattquest
        let notifyBody = jQuery('#notify-variant-body'); //mattequest
        console.log(soldOutDiv);
        ...
        if (variant.available) {
          ...
          // show sold out div and update email body //mattquest
          soldOutDiv.show(); 
          let mq_body = 'Name: ' + variant.name + "\n SKU: " + variant.sku + "\n";
          if(variant.option1)
            mq_body += "Option 1 " + variant.option1 + "\n";
          if(variant.option2)
            mq_body += "Option 2 " + variant.option2 + "\n";
          if(variant.option3)
            mq_body += "Option 3 " + variant.option3 + "\n";
          console.log(mq_body);
          jQuery('#notify-variant-body').val(mq_body);
          ...
          if (this.settings.stockSetting) {
            if (variant.incoming) {
              ...
              cache.$stock.addClass('hide');
              	...
              	console.log('hello')
          console.log($('#agree_nutrition').is(':checked'))
    ...
    setActiveThumbnail: function($el) {
      ...
      $thumbnails.find('.product-single__thumbnail-item').removeClass('slick-center is-active');
      $el.parent().addClass('slick-center');
    ...
    _productZoomImage: function() {
      ...
      $(this.selectors.productImageWrap).on('click' + this.settings.namespace, function(evt) {
        ...
        $(document).find('#ProductModal img').attr('data-slick-index', $(this).attr('data-slick-index'));
      });
      ...
      this.ProductModal.$modal.on('click' + this.settings.namespace, function(evt) {
        if (evt.target.nodeName !== '.modal__inner') {
...
console.log('тут');
    ...
    _productThumbnailSlider: function() {
      ...
      console.log($productThumbWrapper);
      ...
      if (thumbnailCount > 2) {
        ...
        $productThumbWrapper.slick('slickGoTo', $productThumbWrapper.find('.slick-center').attr('data-slick-index'), true);
        /* Ставить основною, активну картинку з списку зліва */
        $('#ProductThumbs-').on('beforeChange', function(event, slick, currentSlide, nextSlide){
        	var $el = $('[data-slick-index="' + currentSlide + '"] a');
        	var sizedImageUrl = $el.attr('href');
       		var largeImageUrl = $el.attr('data-zoom');
            self.setActiveThumbnail($el);
            /* Подивитись до функції, ламає вибір кольором */
            //self.switchProductImage(sizedImageUrl, largeImageUrl);
        });
      }
    },
...
// adding script for tabs in customer account
...
(function($){
  $(document).ready(function(){
   if($(document).find('.product-single__thumbnail-item').length > 0){
   $(document).find('.product-single__thumbnail-item').each(function(){
      $(this).attr('data-slick-index', $(this).index());
   });
    $('#ProductModal .modal__inner').append('<div class="control prev"><i class="fa fa-chevron-left" aria-hidden="true"></i></div><div class="control next"><i class="fa fa-chevron-right" aria-hidden="true"></i></div>');
    $(document).on('click','.product-single__thumbnail-item',function(e){
      $(document).find('#ProductPhotoLink-').attr('data-slick-index', $(this).attr('data-slick-index'));
    });
    
    $(document).on('click','#ProductPhotoLink-',function(e){
      $(document).find('#ProductModal img').attr('data-slick-index', $(this).attr('data-slick-index'));
    });
    
    $(document).on('click', '#ProductModal .next', function(event) {
        var count = $(document).find('.product-single__thumbnail-item').length;
        var this_index = parseInt($(document).find('#ProductModal img').attr('data-slick-index'));
        if(!this_index){this_index = 0;}
        var new_index;
        if((this_index+2) > count){new_index = 0;}else{new_index = this_index+1;}
        var new_el = $(document).find('.product-single__thumbnail-item:eq('+new_index+') a').attr('data-zoom');
        $(document).find('#ProductModal img').attr('data-slick-index', new_index).attr('src', new_el);
      });
      
       $(document).on('click', '#ProductModal .prev', function(event) {
        var count = $(document).find('.product-single__thumbnail-item').length;
        var this_index = parseInt($(document).find('#ProductModal img').attr('data-slick-index'));
        if(!this_index){this_index = 0;}
        var new_index;
        if((this_index) < 1){new_index = count;}else{new_index = this_index;}
        var new_el = $(document).find('.product-single__thumbnail-item:nth-child('+new_index+') a').attr('data-zoom');
        $(document).find('#ProductModal img').attr('data-slick-index', new_index-1).attr('src', new_el);
      });
      
      $(document).mouseup(function (e) {
        if ($('#ProductModal .modal__inner').has(e.target).length === 0){
...
/* show straps promo block for US customers */
$(document).ready(function(){
  function showPromoForUS() {
    try {
      var US_CODE = ["US"];

      $.get(
        "https://api.ipdata.co/?api-key=dd52511ed5ba55dea2e7f7a01fffcabe185b685a538fb06a848f6413",
        function(response) {
          console.log(response)

          if (US_CODE.includes(response.country_code)) {
            $('p.promo-for-us-customers').css('display','inline-block')
          } 
...
/* END show straps promo block for US customers */
